...
def to_chain_structure(qc, setup="linear"):
    ...
    for gate in qc.gates:
        if gate.name == "CNOT" or gate.name == "CSIGN":
            ...
            if setup == "linear" or (setup == "circular" and (end - start) <= N // 2):
                i = start
                while i < end:
                    if start + end - i - i == 1 and (end - start + 1) % 2 == 0:
                        ...
                        if end == gate.controls[0]:
                            qc_t.add_gate(gate.name, targets=[i], controls=[i + 1])
                        else:
                            qc_t.add_gate(gate.name, targets=[i + 1], controls=[i])
                        ...
                        if end == gate.controls[0]:
                            qc_t.add_gate(gate.name, targets=[i + 1], controls=[i + 2])
                        else:
                            qc_t.add_gate(gate.name, targets=[i + 2], controls=[i + 1])
                        ...
                        qc_t.add_gate("SWAP", [i, i + 1])
                        qc_t.add_gate("SWAP", [start + end - i - 1, start + end - i])
                    i += 1
                ...
                while i < (N - end + start):
                    ...
                    if N + start - end - i - i == 1 and (N - end + start + 1) % 2 == 0:
                        if end == gate.controls[0]:
                            temp.add_gate(gate.name, targets=[i], controls=[i + 1])
                        else:
                            temp.add_gate(gate.name, targets=[i + 1], controls=[i])

                    elif (
                        N + start - end - i - i == 2 and (N - end + start + 1) % 2 == 1
                    ):
                        temp.add_gate("SWAP", targets=[i, i + 1])
                        if end == gate.controls[0]:
                            temp.add_gate(gate.name, targets=[i + 2], controls=[i + 1])
                        else:
                            temp.add_gate(gate.name, targets=[i + 1], controls=[i + 2])
                        ...
                        temp.add_gate("SWAP", [i, i + 1])
                        temp.add_gate(
                            "SWAP", [N + start - end - i - 1, N + start - end - i]
                        )
                    i += 1
                ...
                for gate in temp.gates:
                    if j < N - end - 2:
                        if gate.name in ["CNOT", "CSIGN"]:
                            qc_t.add_gate(
                                gate.name, end + gate.targets[0], end + gate.controls[0]
                            )
                        else:
                            qc_t.add_gate(
                                gate.name,
                        ...
                        if gate.name in ["CNOT", "CSIGN"]:
                            ...
                            qc_t.add_gate(
                                gate.name,
                                [end + gate.targets[0], (end + gate.targets[1]) % N],
                            )
                    else:
                        if gate.name in ["CNOT", "CSIGN"]:
                            qc_t.add_gate(
            ...
            if setup == "linear" or (setup == "circular" and (end - start) <= N // 2):
                ...
                for gate in temp.gates:
                    if j < N - end - 2:
                        qc_t.add_gate(
                            gate.name, [end + gate.targets[0], end + gate.targets[1]]
                        )
                    elif j == N - end - 2:
                        qc_t.add_gate(
                            gate.name,
                        ...
                        qc_t.add_gate(
                            gate.name,
                            [(end + gate.targets[0]) % N, (end + gate.targets[1]) % N],
                        )
                    j = j + 1

        else:
            qc_t.add_gate(
                gate.name, gate.targets, gate.controls, gate.arg_value, gate.arg_label
            )
